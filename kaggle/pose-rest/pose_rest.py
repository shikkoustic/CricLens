"""CricLens pose extraction for the ~15.3k clips WITHOUT batter boxes (all sources except CricketVision).

The batter is chosen by the learned batter finder (trained on CricketVision's annotated boxes, with
stumps clues: the striker stands in front of the far stumps). Falls back to the hand-written rules
only if a clip has no usable tracks for the finder. Everything else is the CricketVision method.
(Header of the CricketVision run kept below for the method description.)
...
if SHARD is None:  # main process: install once, launch one worker per GPU, merge results
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", "scikit-learn==1.9.1"], check=False)  # same version that saved the finder
    import torch as _t
    ...
    summ = {"clips": len(res), "usable_rate": float(res.usable.mean()),
            ...
            "finder_confidence_median": float(res.finder_p.median()),
            "low_confidence_clips": int((res.finder_p < 0.5).sum()),
            "by_source_usable": res.groupby("source").usable.mean().round(3).to_dict(),
            "tracks_saved": len(trk),
    ...
    import shutil as _sh  # one packed file downloads far faster and more reliably than ~1,500 small ones
    subprocess.run(["tar", "-cf", "/kaggle/working/kps.tar", "-C", "/kaggle/working", "kps"], check=True)
    _sh.rmtree("/kaggle/working/kps")
    sys.exit(0)
...
CLIPDIR = [c for c in glob.glob("/kaggle/input/**/clips", recursive=True) if os.path.isdir(f"{c}/cricketvision")][0]
...
import joblib
_FP = glob.glob("/kaggle/input/**/batter_finder.joblib", recursive=True)
FINDER = joblib.load(_FP[0]) if _FP else None
_SD = glob.glob("/kaggle/input/**/stumps_dets.csv", recursive=True)
_STD = pd.read_csv(_SD[0]) if _SD else pd.DataFrame(columns=["clip_id", "frame", "cls", "x0", "y0", "x1", "y1"])
ST_BY = {c: {f: g[["x0", "y0", "x1", "y1"]].to_numpy() for f, g in cg.groupby("frame")}
         for c, cg in _STD[_STD.cls == "stumps"].groupby("clip_id")}
FR_BY = _STD.groupby("clip_id")["frame"].unique().to_dict()


def finder_choose(cid, rows, tracks):
    """Score every tracked person with the batter finder; return (track id, probability)."""
    if FINDER is None or not rows:
        return None, float("nan")
    d = pd.DataFrame(rows)
    sf = pd.DataFrame(person_stump_features(tracks, ST_BY.get(cid, {int(f): np.zeros((0, 4)) for f in FR_BY.get(cid, [])})))
    if len(sf):
        d = d.merge(sf, on="tid", how="left")
    for c in ("cy", "h", "striker", "bat"):
        d[f"{c}_rel"] = d[c] - d[c].mean()
    p = FINDER["model"].predict_proba(d.reindex(columns=FINDER["features"]).to_numpy(float))[:, 1]
    j = int(np.argmax(p))
...
def process(path, r):
    ...
    ann = None
    feat_rows = track_features(r.clip_id, tracks, cues, H, W, n, ann)
    tid, finder_p = finder_choose(r.clip_id, feat_rows, tracks)
    if tid is None:                                                    # fallback: hand-written rules
        tid, _ = choose(tracks, cues, H, W, n)
    auto_tid = tid
    ...
    row["finder_p"] = finder_p
    row["_tracks"] = feat_rows
...
_VL = glob.glob("/kaggle/input/**/validation_clips.csv", recursive=True)
if os.environ.get("CRICLENS_VALIDATION") and _VL:   # validation run: only the listed clips (incl. non-canonical CV twins)
    df = m[m.clip_id.isin(pd.read_csv(_VL[0]).clip_id)].sort_values("clip_id").iloc[SHARD::NOF]
else:
...
print(f"finder loaded: {FINDER is not None} | stumps rows: {len(_STD)}", flush=True)
...
for n_, r in enumerate(df.itertuples()):
    ...
    try:
        out = process(f"{CLIPDIR}/{r.source}/{r.clip_id}.mp4", r)
        if out is None:
            continue
        row, fr, k, boxes, (lo, c, hi) = out; trows += row.pop("_tracks"); rows.append(row); nf += len(fr)
        if n_ % 40 == 0 or os.environ.get("CRICLENS_VALIDATION"):  # denser samples for the post-run visual audit
